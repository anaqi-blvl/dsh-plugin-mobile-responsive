...
`;
		...
		/* iOS "Add to Home Screen" icon: point apple-touch-icon at the icon the
		   node half serves. */
		try {
			if (typeof document !== "undefined" && document.querySelector('link[rel="apple-touch-icon"]') === null) {
				const link = document.createElement("link");
				link.rel = "apple-touch-icon";
				link.href = "/dsh-mobile-icon.png";
				document.head.appendChild(link);
			}
		} catch {
			/* icon linking must never break the app */
		}
